...
class Bee(Insect):
    ...
    def __init__(self, armor):
        """Create an bee with an ARMOR quantity."""
...
class ThrowerAnt(Ant):
    ...
    def nearest_bee(self, hive):
        ...
        c_place = self.place
        num_places = 0
        while num_places < self.min_range:
            num_places += 1
            c_place = c_place.entrance
        while (c_place is not hive) and (c_place is not None) and (num_places <= self.max_range):
            num_places += 1
            if c_place.bees:
                return random_or_none(c_place.bees)
            c_place = c_place.entrance
...
class Water(Place):
    ...
    def add_insect(self, insect):
        ...
        if not (insect).watersafe:
            return Insect.reduce_armor(insect, (insect).armor)
        # END Problem 3A
...
# BEGIN Problem 5A
class WallAnt(Ant):
    """WallAnt does nothing but act as a defense to incoming Bees"""
    name = 'Wall'
    food_cost = 4
    implemented = True
...
class HungryAnt(Ant):
    ...
    def action(self, colony):
        ...
        if self.digesting > 0:
            ...
            if random_or_none(self.place.bees) != None:
                self.eat_bee(random_or_none(lst))
                self.digesting = self.time_to_digest

    """def __init__(self, digesting=3):
        # BEGIN Problem 6B
        self.digesting = digesting
        Ant.__init__(self)
        # END Problem 6B
    ...
    def action(self, colony):
        ...
        if self.digesting == self.time_to_digest:
            self.digesting = -1
            if self.place.bees:
                self.eat_bee(random.choice(lst))
        else:
            self.digesting +=1"""
        # END Problem 6B
...
class QueenAnt(ScubaThrower):  # You should change this line
    ...
    def __init__(self):
        # BEGIN Problem 9
        Ant.__init__(self)
        if QueenAnt.instantiated == 0:
            self.true_queen = True
            QueenAnt.instantiated += 1
        else:
    ...
    def action(self, colony):
        ...
        def double(ant):
            square = ant.place
            while square.exit != None:
                square = square.exit
                if square.ant != None:
                    
                    if square.ant.container and square.ant.ant != None:
                        if not (square.ant in self.list_of_doubled):
                            square.ant.damage = square.ant.damage * 2
                            self.list_of_doubled.append(square.ant)

                        if not square.ant.ant in self.list_of_doubled:
                            square.ant.ant.damage = square.ant.ant.damage * 2
                            self.list_of_doubled.append(square.ant.ant)
